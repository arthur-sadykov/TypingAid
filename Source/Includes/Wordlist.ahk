...
ReadWordList()
{
   ...
   if (LoadWordlist) {
      ...
      for each, row in wordsInDB.Rows
      {
        ...
        if (!matchFound) {
            g_WordListDB.Query("DELETE FROM words WHERE word = '" . row[2] . "';")
        }
      }
      ParseWords =
      g_WordListDB.EndTransaction()
...
AddWordToList(AddWord,ForceCountNewOnly,ForceLearn=false, ByRef LearnedWordsCount = false)
{
   ...
   StringLower, wordInLowerCase, AddWordTransformed
   IfEqual, g_WordListDone, 0 ;if this is read from the wordlist
   {
      ...
      t := g_WordListDB.Query("SELECT word FROM Words WHERE word = '" . wordInLowerCase . "';")
      ...
      if (t.Rows.Count() == 0) {
         ...
         g_WordListDB.Query("INSERT INTO words (wordindexed, word, worddescription, wordreplacement) VALUES ('" . AddWordIndexTransformed . "','" . wordInLowerCase . "'," . WordDescriptionQuery . "," . WordReplacementQuery . ");")
      }

   } else if (prefs_LearnMode = "On" || ForceCountNewOnly == 1)
   {
      ; If this is an on-the-fly learned word
      AddWordInList := g_WordListDB.Query("SELECT * FROM words WHERE word = '" . wordInLowerCase . "';")
      ...
      IF !( AddWordInList.Count() > 0 ) ; if the word is not in the list
      {
         ...
         ; must update wordreplacement since SQLLite3 considers nulls unique
         g_WordListDB.Query("INSERT INTO words (wordindexed, word, count, wordreplacement) VALUES ('" . AddWordIndexTransformed . "','" . wordInLowerCase . "','" . CountValue . "','');")
      } else IfEqual, prefs_LearnMode, On
      {
         IfEqual, ForceCountNewOnly, 1
         {
            ...
            {
               g_WordListDB.QUERY("UPDATE words SET count = ('" . prefs_LearnCount . "') WHERE word = '" . wordInLowerCase . "');")
            }
         } else {
